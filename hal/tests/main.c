 ...
 * @brief HAL模块独立测试入口
 *
 * 本文件提供HAL模块的独立测试程序入口，支持：
 * - 独立编译：可单独编译HAL测试程序
 * - 完整功能：支持交互式菜单、命令行参数
 * - 依赖OSAL：HAL层依赖OSAL抽象层
 */
...
static void print_usage(const str_t *prog_name)
{
    ...
    OSAL_Printf("  -a, --all               Run all HAL tests\n");
    OSAL_Printf("  -l, --list              List all HAL tests\n");
    ...
    OSAL_Printf("  %s -m test_gpio         # Run GPIO module tests\n", prog_name);
    OSAL_Printf("  %s -s test_gpio_suite   # Run specific suite\n", prog_name);
    OSAL_Printf("\n");
}
...
int32_t main(int32_t argc, str_t *argv[])
{
    ...
    OSAL_Printf("\n");
    OSAL_Printf("========================================\n");
    OSAL_Printf("  HAL Module Unit Test\n");
