...
int32 HAL_Serial_Open(const char *device, const hal_serial_config_t *config, hal_serial_handle_t *handle)
{
    ...
    if (ctx->fd < 0)
    {
        OSAL_LogError("HAL_Serial", "Failed to open %s: %s", device, strerror(errno));
        OSAL_Free(ctx);
        return OS_ERROR;
    }
    ...
    {
        OSAL_LogError("HAL_Serial", "Failed to get attributes: %s", strerror(errno));
        close(ctx->fd);
        OSAL_Free(ctx);
        return OS_ERROR;
    }
    ...
    {
        OSAL_LogError("HAL_Serial", "Failed to set attributes: %s", strerror(errno));
        close(ctx->fd);
        OSAL_Free(ctx);
        return OS_ERROR;
    }
...
int32 HAL_Serial_Write(hal_serial_handle_t handle, const void *buffer, uint32 size, int32 timeout)
{
    ...
    if (timeout > 0)
    {
        ...
        {
            OSAL_LogError("HAL_Serial", "Select error: %s", strerror(errno));
            return OS_ERROR;
        }
    }
    ...
    if (written < 0)
    {
        OSAL_LogError("HAL_Serial", "Write error: %s", strerror(errno));
        return OS_ERROR;
    }
...
int32 HAL_Serial_Read(hal_serial_handle_t handle, void *buffer, uint32 size, int32 timeout)
{
    ...
    if (nread < 0)
    {
        OSAL_LogError("HAL_Serial", "Read error: %s", strerror(errno));
        return OS_ERROR;
    }
...
int32 HAL_Serial_Flush(hal_serial_handle_t handle)
{
    ...
    {
        OSAL_LogError("HAL_Serial", "Flush error: %s", strerror(errno));
        return OS_ERROR;
    }
