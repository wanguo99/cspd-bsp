...
|------|--------|--------|--------|--------|--------|
| 阶段1: 架构重构 | 2 | 2 | 0 | 0 | 100% |
| 阶段2: 修复严重问题 | 5 | 5 | 0 | 0 | 100% |
| 阶段3: 重构优化 | 9 | 7 | 0 | 2 | 78% |
| 阶段4: 长期改进 | 7 | 0 | 0 | 7 | 0% |
| **总计** | **23** | **14** | **0** | **9** | **61%** |

**历史完成项**: 14个重构项目已完成（见文档末尾"已完成的优化项"）  
**最后更新**: 2026-04-27
...
- [x] **#2 统一条件判断（Yoda条件）** 🔴
  - [x] 更新编码规范文档
  - [x] 编写自动化检查脚本
  - [x] 批量修改：`if (ptr == NULL)` → `if (NULL == ptr)`
  - [x] 批量修改：`if (status == OS_SUCCESS)` → `if (OS_SUCCESS == status)`
  - [x] 人工审查所有改动
  - [x] 在Code Review中强制执行
  - [x] **提交到git master分支** (commit: f6f36ba)

### 阶段2：修复严重问题（1-2周）⏳

- [x] **#3 修复termios重复定义** 🔴
  - [x] 合并 `osal/include/sys/osal_termios.h` 和 `osal/include/net/osal_termios.h`
  - [x] 删除 `osal/include/sys/osal_termios.h`
  - [x] 更新所有引用
  - [x] **提交到git master分支** (commit: 95fb7ab)

- [x] **#4 修复任务管理的DETACHED/JOIN混用** 🔴
  - [x] 修改 `osal/src/posix/ipc/osal_task.c`
  - [x] 改为JOINABLE模式或DETACHED+条件变量
  - [x] 充分测试任务创建和删除
  - [x] **提交到git master分支** (commit: 273caca)

- [x] **#5 修复堆内存统计** 🔴
  - [x] 修改 `osal/src/posix/lib/osal_heap.c`
  - [x] 在OSAL_Free中减少current_usage
  - [x] 添加内存泄漏检测
  - [x] 测试内存统计准确性
  - [x] **提交到git master分支** (commit: a85c1b3)

- [x] **#6 修复死锁检测回调的线程安全** 🔴
  - [x] 修改 `osal/src/posix/ipc/osal_mutex.c`
  - [x] 使用互斥锁或原子操作保护全局变量
  - [x] 测试多线程场景
  - [x] **提交到git master分支** (commit: 43f5df3)

- [x] **#7 添加PCL单元测试** 🔴
  - [x] 创建 `pcl/tests/test_pcl_api.c`
  - [x] 覆盖所有API和边界情况
  - [x] 测试配置注册、查询、验证
  - [x] **提交到git master分支** (commit: c4ba3dc)

### 阶段3：重构优化（2-3周）⏳

- [x] **#8 统一接口命名** 🟡
  - [x] 修改 `osal/include/osal.h`
  - [x] 修改 `osal/include/sys/osal_clock.h`
  - [x] 将所有OS_前缀改为OSAL_
  - [x] 更新所有调用点
  - [x] **提交到git master分支** (commit: d5c3420)

- [x] **#9 实现任务ID重用机制** 🟡
  - [x] 修改 `osal/src/posix/ipc/osal_task.c`
  - [x] 使用64位位图管理ID（支持1-64）
  - [x] 实现allocate_task_id()和release_task_id()
  - [x] 测试ID分配和回收
  - [x] **提交到git master分支** (commit: 48724d0)

- [x] **#10 增强配置验证** 🔴
  - [x] 修改 `pcl/src/pcl_api.c`
  - [x] 新增CAN/UART/I2C/SPI接口配置验证函数
  - [x] 新增MCU/卫星/BMC硬件接口验证
  - [x] 验证设备名、波特率、数据位、校验位等参数合法性
  - [x] 验证I2C从地址范围、SPI模式、1553B RT地址等
  - [x] 所有22个PCL测试通过
  - [x] **提交到git master分支** (commit: 7e31f36)

- [x] **#11 优化配置查询性能** 🟢 **[已取消 - 不需要]**
  - 理由：当前配置数量少（<10个），线性查找性能足够
  - 过早优化，无实际性能瓶颈

- [x] **#12 创建通用外设框架** 🟢 **[已取消 - 不需要]**
  - 理由：MCU/BMC/Satellite已在PDL层有清晰抽象
  - 再封装会增加不必要的复杂度和维护成本

- [x] **#13 统一互斥锁策略** 🟡
  - [x] 修改 `pdl/src/pdl_satellite/pdl_satellite.c`
  - [x] 为卫星服务添加互斥锁保护共享变量
  - [x] 保护callback、user_data、统计计数器
  - [x] MCU和BMC服务已有互斥锁保护
  - [x] 所有46个测试通过
  - [x] **提交到git master分支** (commit: dfa756f)

- [x] **#14 统一日志输出** 🟡
  - [x] 优化所有模块日志接口
  - [x] 统一使用OSAL中定义的LOG_ERROR/LOG_INFO/LOG_WARN/LOG_DEBUG/LOG_FATAL接口
  - [x] ~~优化模块名配置~~（已跳过）
  - [x] 验证：150处日志调用已使用LOG宏
  - [x] **无需提交**（已完成）
...
- [ ] **#16 清理模块内config头文件** 🟡
  - [ ] 删除所有模块内的 `include/config/` 目录
  - [ ] 将配置定义移到主头文件或实现文件中
  - [ ] 更新编码规范文档，不强制要求config头文件
  - [ ] 简化模块结构，减少不必要的文件层次
  - [ ] **提交到git master分支**
...
- **已修复**：统一为JOINABLE模式，简化删除逻辑 (commit: 273caca)
...
- **已修复**：实现内存块追踪机制，Free时正确减少统计 (commit: a85c1b3)
