...
#ifndef HW_CONFIG_TYPES_H
#define HW_CONFIG_TYPES_H
...
typedef struct {
    ...
    /* GPIO控制（可选） */
    hw_gpio_config_t *reset_gpio; /* 复位GPIO */
    hw_gpio_config_t *irq_gpio;   /* 中断GPIO */
} hw_mcu_cfg_t;
...
typedef struct {
    ...
    /* 主通道配置（以太网） */
    struct {
        hw_interface_type_t type; /* 必须是HW_INTERFACE_ETHERNET */
        hw_ethernet_cfg_t   cfg;
    } primary_channel;

    /* 备份通道配置（串口） */
    struct {
        hw_interface_type_t type; /* 必须是HW_INTERFACE_UART */
        hw_uart_cfg_t       cfg;
    } backup_channel;
    ...
    /* GPIO控制（可选） */
    hw_gpio_config_t *power_gpio; /* 电源控制GPIO */
    hw_gpio_config_t *reset_gpio; /* 复位GPIO */
} hw_bmc_cfg_t;
...
typedef struct {
    ...
    hw_gpio_config_t *enable_gpio; /* 使能GPIO */
...
typedef struct {
    ...
    hw_device_type_t device_type; /* 外设类型 */
...
typedef struct {
    ...
    /* 外设映射列表 */
    hw_app_device_mapping_t *device_mappings;
...
typedef struct {
    ...
    /* 外设配置列表（以外设为单位） */
    hw_mcu_cfg_t       **mcus;         /* MCU外设列表 */
    uint32               mcu_count;

    hw_bmc_cfg_t       **bmcs;         /* BMC外设列表 */
    uint32               bmc_count;

    hw_satellite_cfg_t **satellites;   /* 卫星平台接口列表 */
    uint32               satellite_count;

    hw_sensor_cfg_t    **sensors;      /* 传感器外设列表 */
    uint32               sensor_count;

    hw_storage_cfg_t   **storages;     /* 存储设备列表 */
    uint32               storage_count;

    hw_power_domain_t  **power_domains; /* 电源域列表 */
    uint32               power_domain_count;

    /* APP配置列表（新增） */
    hw_app_config_t    **apps;         /* APP配置列表 */
    uint32               app_count;
