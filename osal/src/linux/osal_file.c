...
int32 OSAL_FileOpen(osal_id_t *fd, const char *path, uint32 mode, uint32 flags)
{
    int32 native_flags;
    int32 native_fd;
    uint32 i;
    ...
    if (g_file_mutex == OS_OBJECT_ID_UNDEFINED)
    {
        ...
        if (g_file_mutex == OS_OBJECT_ID_UNDEFINED)
        {
            OSAL_MutexCreate(&g_file_mutex, "FILE_MTX", 0);
        }
        pthread_mutex_unlock(&g_file_init_mutex);
    }

    OSAL_MutexLock(g_file_mutex);
...
int32 OSAL_FileClose(osal_id_t fd)
{
    uint32 index;
...
int32 OSAL_FileRead(osal_id_t fd, void *buffer, uint32 size)
{
    uint32 index;
    int32 native_fd;
    int32 bytes_read;
...
int32 OSAL_FileWrite(osal_id_t fd, const void *buffer, uint32 size)
{
    uint32 index;
    int32 native_fd;
    int32 bytes_written;
...
int32 OSAL_FileSeek(osal_id_t fd, int32 offset, uint32 whence)
{
    uint32 index;
    int32 native_fd;
    int32 native_whence;
...
int32 OSAL_FileSetFlags(osal_id_t fd, uint32 flags)
{
    uint32 index;
    int32 native_fd;
    int32 current_flags;
...
int32 OSAL_FileGetFlags(osal_id_t fd, uint32 *flags)
{
    uint32 index;
    int32 native_fd;
    int32 native_flags;
...
int32 OSAL_FileIoctl(osal_id_t fd, os_ioctl_req_t request, void *arg)
{
    uint32 index;
    int32 native_fd;
    int32 result;
...
int32 OSAL_FileSelect(const osal_id_t *fds, uint32 nfds,
